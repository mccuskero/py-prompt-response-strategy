"""Core StrategyPipeline interface and abstract base class for pipeline implementations."""
...
class StrategyPipeline(ABC):
    ...
    along with LLM client integration. It provides a clean separation between
    the pipeline orchestration logic and the specific strategy implementations.
    
    The pipeline follows a four-stage execution model:
    1. Prompt Generation - Create structured prompts from input data
    2. LLM Response - Generate responses using language models
    3. Response Processing - Parse and structure LLM responses
    4. XML Output - Transform structured data to XML format
    """
    ...
    # Lifecycle hooks that implementations can override
...
class AbstractStrategyPipeline(StrategyPipeline):
    ...
    StrategyPipeline interface with common functionality for initialization,
    validation, and execution flow. It handles the orchestration of the
    three strategy types and LLM client integration.
    ...
    def __init__(
        ...
        """Initialize the abstract pipeline with required strategies and client.
    ...
    def initialize(self) -> None:
        ...
        try:
            ...
            # Validate all components are present (without LLM connection check)
    ...
    def execute(
        ...
        **kwargs
        ...
        try:
            ...
            # Reset stage before building result
            self._current_stage = None
            self._execution_context = None
    ...
    def _validate_components(self) -> None:
        """Validate that all pipeline components are present (without LLM connection check).
